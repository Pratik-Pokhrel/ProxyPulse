...
**Pitfall, confirmed and documented rather than hidden:** this result looks dramatically worse than every other variant, and it is, but not because `ip_hash` is a poor algorithm. `ip_hash` routes by client source IP, and since every request in this test originates from a single k6 host, all traffic was pinned to a single backend rather than distributed across the pool. This was confirmed directly:
...
`ip_hash` would distribute normally in production traffic with many distinct client IPs. This result specifically demonstrates a limitation of testing `ip_hash` with a single-origin load generator, not a flaw in the algorithm itself.

---

## Phase 4: Caching and rate limiting

Config restored to baseline (`least_conn` + `keepalive 64`) before this phase. Load generated with `load-tests/k6-hot-cache.js` against `/api/hot`, a route that deliberately performs an expensive CPU-bound computation per request.
...
- **Redis as a shared cache backend** was considered and ruled out. `proxy_cache` cannot be redirected to Redis directly; doing so would require hand-rolling the entire cache-aside flow in Lua, which was judged not worth the added complexity for this project's scope.
- **JWT-based edge authentication** was excluded to avoid requiring a custom Dockerfile build for the edge image, keeping the stack limited to the base `openresty/openresty:alpine` image plus bundled libraries (`resty.redis`, `resty.upstream.healthcheck`).
- **Distributed rate limiting via Redis** (`resty.redis` + Lua `EVAL`) was scoped as a stretch goal beyond the core roadmap and is documented separately rather than benchmarked here.
